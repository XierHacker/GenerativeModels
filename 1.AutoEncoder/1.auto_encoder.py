...
class AutoEncoder():
            ...
            with tf.variable_scope(name_or_scope=name_scope,reuse=False):
                ...
                with tf.variable_scope(name_or_scope="layer_1",reuse=reuse):
                    weight=tf.get_variable(
                        name="weight",
                        shape=(self.image_flatten_dim,self.code_dim),
                        dtype=tf.float32,
        ...
        with self.graph.as_default():
            with tf.variable_scope(name_or_scope=name_scope,reuse=reuse):
                ...
                with tf.variable_scope("layer_1",reuse=reuse):
                    weight = tf.get_variable(
                        name="weight",
                        shape=(self.code_dim, self.image_flatten_dim),
                        dtype=tf.float32,
                        initializer=tf.initializers.truncated_normal()
                    )
                    bias = tf.get_variable(
    ...
    def fit(self,X_train):
        with self.graph.as_default():
            ...
            #loss
            #self.loss = tf.reduce_mean(tf.pow(self.X_p - generate, 2))
            self.loss=tf.losses.mean_squared_error(
                labels=tf.reshape(self.X_p,[-1]),
        ...
        with self.session as sess:
            ...
            for epoch in range(1, self.max_epoch + 1):
                ...
                if sum(losses)/len(losses)<0.2:
                    #store the first pictre
                    gen_pic=sess.run(
                        fetches=generate,
                        feed_dict={self.X_p:X_train}
                    )
                    #print("gen_pic:\n",gen_pic)
                    plt.imshow(np.reshape(X_train[1],newshape=[28,28]))
...
if __name__=="__main__":
    ...
    X_train,X_test=data.standard(X_train=X_train,X_test=X_test)
    print(X_train.shape)
    print(y_train.shape)
    print(X_test.shape)
    print(X_train[0])
